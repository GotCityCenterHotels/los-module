...
CREATE TABLE IF NOT EXISTS functions.supplement_room_categories (
    hotel_code text NOT NULL REFERENCES functions.supplement_hotels(hotel_code),
...
CREATE TABLE IF NOT EXISTS functions.supplement_snapshot_category (
    ...
    currency text NOT NULL DEFAULT 'SEK' CHECK (currency = 'SEK'),
    run_id bigint NOT NULL REFERENCES functions.supplement_sync_runs(run_id),
    PRIMARY KEY (snapshot_date, stay_date, hotel_code, space_room_category_id),
    CHECK (assigned_rooms >= 0),
    CHECK (room_revenue >= 0)
) PARTITION BY RANGE (snapshot_date);
...
CREATE TABLE IF NOT EXISTS functions.supplement_snapshot_inventory (
    ...
    inventory_quality text NOT NULL
        CHECK (inventory_quality IN ('exact', 'approximated-current')),
    run_id bigint NOT NULL REFERENCES functions.supplement_sync_runs(run_id),
    PRIMARY KEY (snapshot_date, stay_date, hotel_code, space_room_category_id),
    CHECK (total_space >= 0),
...
CREATE TABLE IF NOT EXISTS functions.supplement_latest_detail (
    ...
    run_id bigint NOT NULL REFERENCES functions.supplement_sync_runs(run_id),
    PRIMARY KEY (
        stay_date, hotel_code, space_room_category_id, requested_room_category_id
    )
);
